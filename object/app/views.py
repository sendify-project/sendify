...
def files():
    ...
    response = s3.Bucket('sendify-object').put_object(Key=fid, Body=file,
                                                      ACL='public-read', Expires=datetime.datetime.today()+datetime.timedelta(days=expire_days), ContentType=content_type).get()  # ['ResponseMetadata']['HTTPStatusCode']
    if not response:
